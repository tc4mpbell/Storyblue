package script
{
	...
	public class Story
	{
		...
		public function set CurrentNode(value:XML):void
		{
			if(value == null)
			{
				Application.application.btnDeleteCard.enabled = false;
				Application.application.textNote.enabled = false;
				Application.application.textNoteTitle.enabled = false;
				Application.application.textNoteTitle.prompt = "";
				Application.application.btnAddChar.setFocus();
			}
			else
			{
				lastNode = _currentNode;
				
				Application.application.btnDeleteCard.enabled = true;
				
				//enable note textboxes
				Application.application.textNote.enabled = true;
				Application.application.textNoteTitle.enabled = true;
				Application.application.textNoteTitle.setFocus();
				//set other list's selected indexes to -1
				
				//find currently open list in accordion.
				var curListIx:int = Application.application.accPlan.selectedIndex;
				
				var sc = Application.application.listScenes;
				var no = Application.application.listNotes;
				//var no = Application.application.treeNotes;
				var ch = Application.application.listChars;
				
				if(curListIx == 0) //characters
				{
					Application.application.lblCardTitle.text = "Character Name";
					//-1 on scenes, notes
				...
				{
					Application.application.lblCardTitle.text = "Scene Title";
					//-1 on chars, notes
					if(no!=null) no.selectedIndex = -1;
				...
				{
					Application.application.lblCardTitle.text = "Note Title";
					//-1 on scenes, chars
					if(ch!=null) ch.selectedIndex = -1;
					if(sc!=null) sc.selectedIndex = -1;
				}
		...
		*/
		...
		public function AddNode(type:String):void
		{
			...
			var acc:Accordion = Application.application.accPlan;
		...
		public function DeleteNode(title:String):void
		{
			...
			{
				//get type 
				var type:String = mx.core.Application.application.story.xml.cards.card.(@title==title)[0].@type;
				
				delete mx.core.Application.application.story.xml.cards.card.(@title==title)[0];
